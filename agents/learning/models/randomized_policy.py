...
class ActorCritic(nn.Module):
  ...
  def __init__(self, num_inputs: int, num_actions: int, device: torch.device):
    ...
    self.actor = nn.Sequential(
        init_(nn.Linear(222, hs_config.PPOAgent.hidden_size)),
        nn.ReLU(),
  ...
  def parse_zone(self, zone):
    zone = torch.stack(zone).permute(1, 0, 2)  # position, batch, dims -> batch, position, dim
    sparse = zone[:, :, -1]
    dense = zone[:, :, :-1]

    ids = []
    for card_id in sparse.cpu().view(-1):
      card_index = self.card_id_to_index[int(card_id)]
      ids.append(card_index)

    sparse = torch.LongTensor(ids).view(sparse.shape)
    card_embeddings = self.card_embedding(sparse.long()).cuda()
    zone = torch.cat((dense, card_embeddings), dim=-1)
    return zone.flatten(start_dim=1)  # turn into batch_size x zone
